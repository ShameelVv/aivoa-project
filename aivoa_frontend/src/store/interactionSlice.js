...
// Save interaction from chat (after AI fills form)
export const saveFromChat = createAsyncThunk(
  "interactions/saveFromChat",
  async (formData) => {
...
const interactionSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(saveFromChat.pending, (state) => {
        state.loading = true
      })
      .addCase(saveFromChat.fulfilled, (state, action) => {
        state.loading = false
        state.list.unshift(action.payload)
...
export const { addUserMessage, clearChat, clearFormData } =
